...
void ppi_for_txrx_set(bool is_tx, bool switch_mode)
{
	...
	uint32_t radio_crcok_event = nrf_radio_event_address_get(NRF_RADIO, NRF_RADIO_EVENT_CRCOK);
	...
	if (switch_mode) {
		...
		nrf_ppi_channel_endpoint_setup(NRF_PPI, radio_dis_en, radio_disabled_event,
					       timer_shutdown_task);
	}
	...
	if (!is_tx) {
		...
		nrf_ppi_channel_endpoint_setup(NRF_PPI, radio_dis_trigger, radio_crcok_event,
					       timer_shutdown_task);
		// uint32_t cc3_capture_task =
		// 	nrf_timer_task_address_get(ESB_NRF_TIMER_INSTANCE, NRF_TIMER_TASK_CAPTURE3);
		// nrf_ppi_channel_and_fork_endpoint_setup(NRF_PPI, radio_dis_trigger,
		// 					radio_crcok_event, timer_shutdown_task,
		// 					cc3_capture_task);
	}
